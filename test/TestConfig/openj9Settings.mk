...
ifneq (, $(findstring current, $(JCL_VERSION)))
	ifneq (, $(findstring 8, $(JDK_VERSION)))
		ifneq (,$(findstring win,$(SPEC)))
			JAVA_SHARED_LIBRARIES_DIR:=$(TEST_JDK_HOME)$(D)jre$(D)bin$(D)$(VM_SUBDIR)
			J9VM_PATH=$(TEST_JDK_HOME)$(D)jre$(D)bin$(D)j9vm
		else
			JAVA_SHARED_LIBRARIES_DIR:=$(TEST_JDK_HOME)$(D)jre$(D)lib$(D)$(ARCH_DIR)$(D)$(VM_SUBDIR)
			J9VM_PATH=$(TEST_JDK_HOME)$(D)jre$(D)lib$(D)$(ARCH_DIR)$(D)j9vm
		endif
		ADD_JVM_LIB_DIR_TO_LIBPATH:=export LIBPATH=$(Q)$(LIBPATH)$(P)$(TEST_JDK_HOME)$(D)jre$(D)lib$(D)$(VM_SUBDIR)$(P)$(JAVA_SHARED_LIBRARIES_DIR)$(P)$(TEST_JDK_HOME)$(D)jre$(D)bin$(D)j9vm$(Q);
	else
		ifneq (,$(findstring win,$(SPEC)))
	...
	ifneq (, $(findstring 8, $(JDK_VERSION)))
		ifneq (,$(findstring win,$(SPEC)))
			...
			VM_SUBDIR_PATH=$(TEST_JDK_HOME)$(D)jre$(D)lib$(D)$(ARCH_DIR)$(D)$(VM_SUBDIR)
			J9VM_PATH=$(TEST_JDK_HOME)$(D)jre$(D)lib$(D)$(ARCH_DIR)$(D)j9vm
		endif
	else
		ifneq (,$(findstring win,$(SPEC))) 
