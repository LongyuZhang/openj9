...
JAVA_8_LIB_DIR:=$(TEST_JDK_HOME)$(D)jre$(D)lib
JAVA_8_BIN:=$(TEST_JDK_HOME)$(D)jre$(D)bin
VM_SUBDIR=default
...
ifneq (, $(findstring current, $(JCL_VERSION)))
	ifneq (,$(findstring win,$(SPEC)))
		JAVA_SHARED_LIBRARIES_DIR:=$(JAVA_8_BIN)$(D)$(VM_SUBDIR)
		J9VM_PATH=$(JAVA_8_BIN)$(D)j9vm
	else
		JAVA_SHARED_LIBRARIES_DIR:=$(JAVA_8_LIB_DIR)$(D)$(ARCH_DIR)$(D)$(VM_SUBDIR)
		J9VM_PATH=$(JAVA_8_LIB_DIR)$(D)$(ARCH_DIR)$(D)j9vm
	endif
	ADD_JVM_LIB_DIR_TO_LIBPATH:=export LIBPATH=$(Q)$(LIBPATH)$(P)$(JAVA_8_LIB_DIR)$(D)$(VM_SUBDIR)$(P)$(JAVA_SHARED_LIBRARIES_DIR)$(P)$(JAVA_8_BIN)$(D)j9vm$(Q);
else
	ifneq (, $(findstring 8, $(JDK_VERSION)))
		ifneq (,$(findstring win,$(SPEC)))
			VM_SUBDIR_PATH=$(JAVA_8_BIN)$(D)$(VM_SUBDIR)
			J9VM_PATH=$(JAVA_8_BIN)$(D)j9vm
		else
			VM_SUBDIR_PATH=$(JAVA_8_LIB_DIR)$(D)$(ARCH_DIR)$(D)$(VM_SUBDIR)
			J9VM_PATH=$(JAVA_8_LIB_DIR)$(D)$(ARCH_DIR)$(D)j9vm
		endif
	else
		ifneq (,$(findstring win,$(SPEC))) 
